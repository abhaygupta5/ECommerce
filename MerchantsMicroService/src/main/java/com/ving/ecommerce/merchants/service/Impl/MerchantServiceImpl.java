...
import com.ving.ecommerce.merchants.model.*;
...
public class MerchantServiceImpl implements MerchantService {
    ...
    public ResponseObject getMerchantList(int productId) {
        ...
        List<ProductMerchantWithNameDTO> productMerchantWithNameDTOS  = new ArrayList<>();
        System.out.println(productMerchants);
        if(productMerchants.size() != 0){
            for(ProductMerchant productMerchant: productMerchants){
                int merchantId = productMerchant.getMerchantId();
                productMerchantWithNameDTOS.add(new ProductMerchantWithNameDTO(merchantId, merchantRepository.findOne(merchantId).getMerchantName(),productId, productMerchant.getStock(),(double)getAverageMerchantRating(merchantId).getData(),productMerchant.getProductPrice()));

            }
            return new ResponseObject(productMerchantWithNameDTOS, true);
        }
    ...
    public void updateTopMerchantsTable(){
        ...
        for(int productId : productList){
            ...
            for(int merchantId : merchantListForProduct){
                ...
                double merchantScore = (numberOfProductsMerchantSelling + currentStockOfProduct + averageMerchantRating + 0.2*priceOfProduct + orderCount)/5;
